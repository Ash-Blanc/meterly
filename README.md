...
**A cost-aware sidecar agent for TrueForge.** Meterly sits next to any agent and whispers the price tag before every expensive action — then pauses for human approval when the spend gets serious. It never does the task itself. It makes the agent that does the task smarter about money.

Built for [The Agent Harness Hackathon](https://www.wemakedevs.org/hackathons/trueforge) (WeMakeDevs × TrueFoundry).

## The problem

Agents burn tokens invisibly. You ask for a "quick comparison," the agent spawns four subagents, fans out twenty tool calls, and you find out what it cost when the invoice arrives. Every agent framework treats cost as an observability problem — something you review *after*. Meterly treats it as a **control problem** — something the agent must reason about *before* acting.
...
Meterly doesn't wait for agents to opt in. `scan_local_harnesses` detects coding-agent CLIs installed on the machine and **ingests their real session history**, then `forecast_bill` projects the actual bill from it:
...
Every cost is computed in **real units**: published $/MTok list prices per token class (input / output / cacheRead / cacheWrite), fuzzy model resolution (`openai/gpt-4o-mini-2024-07` → `gpt-4o-mini`), cache discounts applied, negotiated rates via `METERLY_PRICING_JSON`. The dashboard's **Connected Harnesses** and **Bill Forecast** panels render this live — daily burn, 30-day projection, confidence.

Because it's plain MCP, any harness that speaks MCP can mount Meterly — Claude Code, Cursor, Codex, CrewAI, anything. TrueForge is the reference host, not the boundary.
...
1. **Price tag before action** — main agent proposes a 4-subagent research fan-out; Meterly estimates $0.24 before anything runs.
2. **Cheaper path** — Meterly suggests consolidating to 2 subagents (saves ~45%); the agent takes the advice.
...
- **MCP tools**: the entire Meterly surface is a real MCP server over Streamable HTTP
...
- **Subagents**: main agent's fan-out is the thing Meterly prices and optimizes
...
- Representative PR: https://github.com/<you>/meterly/pull/3
